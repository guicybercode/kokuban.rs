...
def main():
    ...
    require((base / 'audit-source.py').read_bytes() == files[(34748686411, None, 'audit-row-baseline-frame-34748686411.py')], 'audit source copy')
    require(summary['decision'] == 'REJECT_THIS_VARIANT', 'decision scope')
    ...
    for run in manifest['runs']:
        run_id = run['id']
        metadata = json.loads(files[(run_id, None, 'github-run.json')])
        ...
        for arch in run['architectures']:
            ...
            for side in ('before', 'after'):
                require(raw(f'{side}-revision.txt').decode().strip() == run['architectures'][arch]['source_revisions'][side], 'source revision')
                require(read(f'injection/{side}/original-source-manifest.json') == read(f'injection/{side}/prepared-source-manifest.json'), 'source manifests')
            before_manifest = read('injection/before/original-source-manifest.json')
            after_manifest = read('injection/after/original-source-manifest.json')
            require(before_manifest.keys() == after_manifest.keys(), 'source file sets')
            require([name for name in before_manifest if before_manifest[name] != after_manifest[name]] == ['src/linux_window.rs'], 'isolated source delta')
            require(raw('harness-revision.txt').decode().strip() == run['harness_revision'], 'harness revision')
            parsed = []
            for pair, side in order:
                text = raw(f'measurements/{pair:02d}-{side}.log').decode()
                filtered = 733 if side == 'before' else 734
                require(len(re.findall(r'^test result: ok\. 1 passed; 0 failed; 0 ignored; 0 measured; ' + str(filtered) + r' filtered out;', text, re.M)) == 1, 'incomplete process log/test count')
            ...
            for case in report['summary']:
                ...
                require([min(changes), max(changes)] == retained['paired_time_change_range_percent'], 'outlier range')
                require(statistics.median(changes[::2]) == retained['AB_median_time_change_percent'] and statistics.median(changes[1::2]) == retained['BA_median_time_change_percent'], 'order medians')
                require(statistics.median([(a-b)/1000 for b,a in zip(values['before'],values['after'])]) == retained['median_paired_time_delta_us'], 'absolute paired delta')
    ...
    require((log_count, record_count, image_count) == (24, 288, 48), 'evidence count')
