...
def audit(args):
    ...
    for name, expected in [('BEFORE_REF', before), ('AFTER_REF', after), ('SCREEN', args.profile)]:
        ...
        if name == 'SCREEN':
            require(observed == {expected}, f'CI screen mismatch: {sorted(observed)}')
        else:
            require(len(observed) == 1, f'CI {name} inconsistent inputs: {sorted(observed)}')
            value = next(iter(observed))
            require(re.fullmatch(r'[0-9a-f]{7,40}', value) and expected.startswith(value)
                    and revision(repo, value) == expected, f'CI {name} does not uniquely resolve to {expected}: {value}')
            marker = 'git rev-parse --verify --end-of-options "${' + name + '}^{commit}"'
            require(marker in snapshots[WORKFLOW].decode() and marker in log,
                    f'CI {name} must resolve the immutable input before archiving')
    ...
    manifest = {'schema': 1, 'kind': 'audited-paired-compact-history', 'before_revision': before,
