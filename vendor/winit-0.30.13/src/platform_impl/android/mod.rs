// Kokuban local Android input corrections; see vendor/README.md.
use std::cell::Cell;
...
use android_activity::input::{
    Axis, InputEvent, KeyAction, KeyMapChar, Keycode, MetaState, MotionAction,
};
...
use crate::keyboard::{Key, ModifiersKeys, ModifiersState};
...
impl<T: 'static> EventLoop<T> {
    pub(crate) fn new(
        ...
        Ok(Self {
            ...
            input_state: input_state::InputState::default(),
        })
    }
    ...
    fn update_modifiers<F>(&mut self, meta: MetaState, callback: &mut F)
    where
        F: FnMut(event::Event<T>, &RootAEL),
    {
        let Some(meta) = self.input_state.modifiers_changed(meta.0) else {
            return;
        };
        let meta = MetaState(meta);
        let mut state = ModifiersState::empty();
        state.set(ModifiersState::SHIFT, meta.shift_on());
        state.set(ModifiersState::CONTROL, meta.ctrl_on());
        state.set(ModifiersState::ALT, meta.alt_on());
        state.set(ModifiersState::SUPER, meta.meta_on());
        let mut pressed_mods = ModifiersKeys::empty();
        pressed_mods.set(ModifiersKeys::LSHIFT, meta.shift_left_on());
        pressed_mods.set(ModifiersKeys::RSHIFT, meta.shift_right_on());
        pressed_mods.set(ModifiersKeys::LCONTROL, meta.ctrl_left_on());
        pressed_mods.set(ModifiersKeys::RCONTROL, meta.ctrl_right_on());
        pressed_mods.set(ModifiersKeys::LALT, meta.alt_left_on());
        pressed_mods.set(ModifiersKeys::RALT, meta.alt_right_on());
        pressed_mods.set(ModifiersKeys::LSUPER, meta.meta_left_on());
        pressed_mods.set(ModifiersKeys::RSUPER, meta.meta_right_on());
        callback(
            event::Event::WindowEvent {
                window_id: window::WindowId(WindowId),
                event: event::WindowEvent::ModifiersChanged(event::Modifiers {
                    state,
                    pressed_mods,
                }),
            },
    ...
    fn mouse_button_event<F>(&self, device: i32, button: u32, pressed: bool, callback: &mut F)
    where
        F: FnMut(event::Event<T>, &RootAEL),
    {
        let button = match button {
            1 => event::MouseButton::Left,
            2 => event::MouseButton::Right,
            4 => event::MouseButton::Middle,
            8 => event::MouseButton::Back,
            16 => event::MouseButton::Forward,
            _ => return,
        };
        callback(
            event::Event::WindowEvent {
                ...
                event: event::WindowEvent::MouseInput {
                    device_id: event::DeviceId(DeviceId(device)),
                    state: if pressed {
    ...
    fn reset_input<F>(&mut self, callback: &mut F)
    where
        F: FnMut(event::Event<T>, &RootAEL),
    {
        for (device, buttons) in self.input_state.release_all() {
            // Leave before releasing so applications cancel clicks/dragging
            // rather than treating lifecycle cancellation as a completed click.
            callback(
                event::Event::WindowEvent {
                    window_id: window::WindowId(WindowId),
            ...
            for bit in [1, 2, 4, 8, 16] {
                if buttons & bit != 0 {
                    self.mouse_button_event(device, bit, false, callback);
                }
            }
        }
        self.combining_accent = None;
        self.update_modifiers(MetaState(0), callback);
    }
        ...
        F: FnMut(event::Event<T>, &RootAEL),
    {
        ...
        match event {
            InputEvent::MotionEvent(motion_event) => {
                ...
                self.update_modifiers(motion_event.meta_state(), callback);

                if input_state::is_mouse(motion_event.source().into()) {
                    ...
                    let cancel = matches!(action, MotionAction::Cancel | MotionAction::HoverExit);
                    if action == MotionAction::HoverEnter || cancel {
                        callback(
                            event::Event::WindowEvent {
                                window_id,
                                event: if cancel {
                    ...
                    // Input consumers need the event's coordinates before a
                    // click or wheel event, including a click without hover.
                    if motion_event.pointer_count() > 0 && !cancel {
                    ...
                    for (button, pressed) in self.input_state.buttons_changed(
                        motion_event.device_id(),
                        motion_event.button_state().0,
                        action == MotionAction::Down,
                        matches!(action, MotionAction::Up | MotionAction::ButtonRelease),
                        cancel,
                    ) {
                    ...
                    if action == MotionAction::Scroll && motion_event.pointer_count() > 0 {
                        let pointer = motion_event.pointer_at_index(0);
                        // Android's horizontal axis moves content left for a
                        // positive value; winit's positive direction is right.
                        if let Some((x, y)) = input_state::scroll_delta(
                ...
                // Relative mice and navigation devices are not touch contacts.
                // Preserve the existing touchscreen/stylus path below.
            ...
            InputEvent::KeyEvent(key) => {
                self.update_modifiers(key.meta_state(), callback);
                match key.key_code() {
                    ...
                    keycode => {
                        ...
                        // logical_key excludes Ctrl (winit's cross-platform
                        // contract). Keep Shift/Alt/lock bits for keyboard layouts.
                        let key_char = if key.meta_state().0 & input_state::CTRL_MASK != 0 {
                            android_app
                                ...
                                .and_then(|map| {
                                    map.get(
                                        keycode,
                                        MetaState(key.meta_state().0 & !input_state::CTRL_MASK),
                                    )
                                    .ok()
                                })
                                .filter(|character| matches!(character, KeyMapChar::Unicode(_)))
                                .or(key_char)
                        } else {
                            key_char
                        };
                        // Android often provides Unicode for Tab/Enter/Escape.
                        // Retain their named identity so e.g. Shift+Tab remains
                        // a modified Tab, not an ordinary text tab character.
                        let logical_key = match keycodes::to_logical(None, keycode) {
                            named @ Key::Named(_) => named,
                            _ => keycodes::to_logical(key_char, keycode),
                        };
