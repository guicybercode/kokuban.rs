...
impl AndroidRuntime {
    ...
    /// Invoke the APK's extracted, read-only Rust SSH executable through mksh.
    /// Android forbids executing downloaded binaries in writable app storage.
    pub fn enable_native_tools(&mut self, native_library_dir: &Path) -> io::Result<()> {
        let executable = fs::canonicalize(native_library_dir.join("libkokuban_ssh.so"))?;
        let metadata = fs::metadata(&executable)?;
        if !metadata.is_file() || metadata.permissions().mode() & 0o111 == 0 {
            return Err(io::Error::new(io::ErrorKind::PermissionDenied, "packaged SSH is not executable"));
        }
        let startup = self.config_path.with_file_name("android-shell.rc");
        let temporary = startup.with_extension(format!("rc.{}", std::process::id()));
        let mut file = OpenOptions::new().write(true).create_new(true).mode(0o600).open(&temporary)?;
        let result = (|| {
            // No path is interpolated into shell source. Quoted environment and
            // argument expansion preserve spaces and shell metacharacters.
            file.write_all(b"# Generated by Kokuban. Customize $HOME/.kokubanrc instead.\nssh() { \"$KOKUBAN_SSH\" \"$@\"; }\n[ ! -r \"$HOME/.kokubanrc\" ] || . \"$HOME/.kokubanrc\"\n")?;
            file.sync_all()?;
            fs::rename(&temporary, &startup)
        })();
        if result.is_err() { let _ = fs::remove_file(&temporary); }
        result?;
        self.environment.retain(|(name, _)| name != "KOKUBAN_SSH" && name != "ENV");
        self.environment.push(("KOKUBAN_SSH".into(), executable.into_os_string()));
        self.environment.push(("ENV".into(), startup.into_os_string()));
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn native_tools_preserve_user_configuration_and_do_not_interpolate_paths() {
        let directory = TestDirectory::new();
        let libs = directory.0.join("native libs ' quoted");
        fs::create_dir(&libs).unwrap();
        let executable = libs.join("libkokuban_ssh.so");
        fs::write(&executable, "#!/bin/sh\nexit 0\n").unwrap();
        fs::set_permissions(&executable, fs::Permissions::from_mode(0o755)).unwrap();
        let mut runtime = AndroidRuntime::prepare(&directory.0).unwrap();
        let user_rc = runtime.home_dir().join(".kokubanrc");
        fs::write(&user_rc, "# user-owned\n").unwrap();
        runtime.enable_native_tools(&libs).unwrap();
        runtime.enable_native_tools(&libs).unwrap();
        let startup = runtime.config_path().with_file_name("android-shell.rc");
        let script = fs::read_to_string(&startup).unwrap();
        assert!(script.contains("\"$KOKUBAN_SSH\" \"$@\""));
        assert!(!script.contains("native libs"));
        assert_eq!(fs::read_to_string(user_rc).unwrap(), "# user-owned\n");
        assert_eq!(fs::metadata(startup).unwrap().permissions().mode() & 0o777, 0o600);
        assert_eq!(runtime.environment.iter().filter(|(key, _)| key == "KOKUBAN_SSH").count(), 1);
        assert_eq!(runtime.environment.iter().find(|(key, _)| key == "KOKUBAN_SSH").unwrap().1,
            fs::canonicalize(executable).unwrap().into_os_string());
    }

    #[test]
    fn missing_native_tools_do_not_replace_shell_environment() {
        let directory = TestDirectory::new();
        let mut runtime = AndroidRuntime::prepare(&directory.0).unwrap();
        let before = runtime.environment.clone();
        assert!(runtime.enable_native_tools(&directory.0).is_err());
        assert_eq!(runtime.environment, before);
        assert!(!runtime.config_path().with_file_name("android-shell.rc").exists());
    }
}
